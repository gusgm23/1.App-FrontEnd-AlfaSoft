...
import {
  controlarCampoCantidad,
  controlarCampoPeriodo,
  validarCamposVaciosSolicitud,
  validarCamposLlenosSolicitud,
} from "../../helpers/validarForms";
...
import {
  ...
  createSolicitud,
  updateSolicitudId,
} from "../../service/apiSolicitudAulas";
...
export const FormularioReservaAula = ({
  ...
  const [isOpenModalConfirm, openModalConfirm, closeModalConfirm] =
    useModal(false);
  const [isOpenModalWarning, openModalWarning, closeModalWarning] =
    useModal(false);
  const [isOpenModalSuccess, openModalSuccess, closeModalSuccess] =
    useModal(false);
  const [isOpenModalFormVacio, openModalFormVacio, closeModalFormVacio] =
    useModal(false);
  ...
  const guardarDatosFormulario = () => {
    ...
    if (idsolicitud === "") {
      createSolicitud(
        ...
        selectsGrupos,
        selectMotivo,
        JSON.stringify(fechaSolicitud).substring(1, 11),
        selectHora,
        "pendiente",
        "ninguno",
        "0",
  ...
  useEffect(() => {
    setListaMateriasDocente(
      obtenerMaterias(data, listaGrupos.data, user.idDocente)
    );
  }, [data, listaGrupos.data]);

  useEffect(() => {
    setListaGruposDocente(
      obtenerGrupos(listaGrupos.data, data, selects, user.idDocente)
    );
  }, [selects]);
  ...
  return (
    <div className="contenedor-reserva-aulas">
      <h1 className="titulo-reserva-aulas">
        {titulo === ""
      ...
      </h1>
      {
            ...
            <div className="contenedor-reserva">
              <div className="contenedor-elementos-reserva-aulas">
                ...
                <div className="campos-reserva-aulas">
                  ...
                  <div className="contenedor-inputs">
                    <GruposDocente
                      datas={listaGruposDocente}
                      selectsGrupos={selectsGrupos}
                      setSelectsGrupos={setSelectsGrupos}
                    />
                    <p
                      className={
                        StatusInputGrupo
                ...
                <div className="campos-reserva-aulas">
                  ...
                  <div className="contenedor-inputs">
                    ...
                    >
                      ...
                      <option value="Examen Primer Parcial">
                        {" "}
                        Examen Primer Parcial{" "}
                      </option>
                      <option value="Examen Segundo Parcial">
                        {" "}
                        Examen Segundo Parcial{" "}
                      </option>
                      <option value="Examen Final"> Examen Final </option>
                      <option value="Examen Segunda Instancia">
                        {" "}
                        Examen Segunda Instancia{" "}
                      </option>
                      <option value="Examen de Mesa 1ra Opción">
                        {" "}
                        Examen de Mesa 1ra Opción{" "}
                      </option>
                      <option value="Examen de Mesa 2ra Opción">
                        {" "}
                        Examen de Mesa 2ra Opción{" "}
                      </option>
                    </select>
                    <p
                      className={
                ...
                <div className="btns-reserva-aula">
                  <button
                    ...
                    type="button"
                    onClick={
                      nomDocente === "" ? volverAtrasSolicitud : closeModal
                    }
                  >
                    Cancelar
                  </button>
        ...
        ) :<p className="parrafoMateriasDocenteVacio">
   
        No tienes materias asignadas para hacer una solicitud. Debes esperar que
        el administrador te asigne una materia.
      </p> 
      :
          <Spinner />
      ...
      <ModalGenerico isOpen={isOpenModalConfirm} closeModal={closeModalConfirm}>
        <Confirmacion
          ...
          funcGuardar={guardarDatosFormulario}
        />
      </ModalGenerico>
